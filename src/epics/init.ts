...
import { ignoreElements, pluck, tap, filter } from 'rxjs/operators';
...
export const loadLevelFromQueryParam: Epic = (action$, { store }) => {
	...
	if (levelId) {
		return ajax(`https://assets.bombhopper.io/levels/${levelId}.json`).pipe(
			pluck('response'),
			filter(() => window.confirm('The saved level will be overwritten. Are you sure?')),
			tap((level) => {
				const { top, right, bottom, left } = getBounds(level.entities);
				const middle = {
					x: left + (right - left) / 2,
					y: top + (bottom - top) / 2,
				};
